...
void draw_points(t_data *data, int *x0, int *y0, int x1, int y1, int color, t_line_params *params)
{
    ...
    // Determine sx valueen = row_len;break;
        e2 = params->err;
...
typedef struct
{
    ...
    double ray_dx;
    double ray_dy;
    double distance;   // Adding distance to the ray struct
} t_ray;
...
void cast_ray(t_data *data, t_player *player, double angle)
{
    ...
    double constant_projection_value = 500;  // you can adjust this value
    int wall_slice_height = (int)(constant_projection_value / ray.distance);

    // Calculate the starting and ending y-coordinates for drawing the wall slice
    int draw_start = (40 * data->map.row_len) / 2 - wall_slice_height / 2;
    if (draw_start < 0) draw_start = 0;
    int draw_end = (40 * data->map.row_len) / 2 + wall_slice_height / 2;
    if (draw_end >= 40 * data->map.row_len) draw_end = 40 * data->map.row_len - 1;

    // Draw the wall slice for this ray on the screen
    int screen_x = player->x + cos(angle) * ray.distance;
    for (int y = draw_start; y <= draw_end; y++) 
    {
        my_mlx_pixel_put(data, screen_x, y, 0xFFFFFF);  // Color of the wall
    }
}
...
//     }
//     ray.distance = sqrt((ray.ray_x - player->x) * (ray.ray_x - player->x) + 
//                         (ray.ray_y - player->y) * (ray.ray_y - player->y));  // calculate distance
    
//     // You can now use ray.distance for wall height calculations
//     // Let's just draw the ray for now:
