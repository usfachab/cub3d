...
/*   Updated: 2023/08/27 20:53:26 by yachaab          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	setup_texture(t_config *c)
{
	...
	if (!c->nt.addr)
		return (0);
	c->st.img = mlx_xpm_file_to_image(c->mlx, c->south_path, &(c->st.width), &(c->st.height));
	if (!c->st.img)
		return (0);
	c->st.addr = mlx_get_data_addr(c->st.img, &(c->st.bits_per_pixel), &c->st.line_length, &(c->st.endian));
	if (!c->st.addr)
		return (0);
	c->et.img = mlx_xpm_file_to_image(c->mlx, c->east_path, &(c->et.width), &(c->et.height));
	if (!c->et.img)
		return (0);
	c->et.addr = mlx_get_data_addr(c->et.img, &(c->et.bits_per_pixel), &c->et.line_length, &(c->et.endian));
	if (!c->et.addr)
		return (0);
	c->wt.img = mlx_xpm_file_to_image(c->mlx, c->west_path, &(c->wt.width), &(c->wt.height));
	if (!c->wt.img)
		return (0);
