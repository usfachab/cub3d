...
void	door(t_player *player, t_map *map, double direction)
{
	...
	new_y = player->y - sin(direction) * CELL_SIZE;
	if (is_valid_position(map, new_x, player->y) || is_valid_position(map, player->x, new_y))
	{
		if (map->map[(int)new_y / CELL_SIZE][(int)new_x / CELL_SIZE] == 'D')
			map->map[(int)new_y / CELL_SIZE][(int)new_x / CELL_SIZE] = 'R';
		else if (map->map[(int)new_y / CELL_SIZE][(int)new_x / CELL_SIZE] == 'R')
